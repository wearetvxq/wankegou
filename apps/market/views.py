...
# Create your views here.
from users.models import UserProfile, JiaoYiJiLu, ChouJiang
...
import hashlib
import re
from utils.login import LoginRequiredMixin
        ...
        if dog.status == 'zhengchang':
            ...
            if user.yue > Decimal(dog.jiage):
                ...
                if type == -1:
                    ...
                    if dog.daishu == 0:
                        daishu = mudog.daishu + 1
                    if mudog.daishu == 0:
                        daishu = dog.daishu + 1
                    else:
                        daishu = int(dog.daishu) + int(mudog.daishu)


                    user.save()
                    userid = user.id

                    nowid = getdogs(userid, daishu)
